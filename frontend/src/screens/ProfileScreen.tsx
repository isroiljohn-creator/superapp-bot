        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border/50 rounded-2xl p-4 mb-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-primary font-medium mt-1">{getGoalLabel()}</p>
            </div>
          </div>

          <div className="grid grid-cols-4 gap-2">
            <div className="text-center p-2 rounded-xl bg-muted/50">
              <p className="text-base font-bold text-foreground">{profile?.age || 25}</p>
              <p className="text-[10px] text-muted-foreground truncate">{labels.age}</p>
            </div>
            <div className="text-center p-2 rounded-xl bg-muted/50">
              <p className="text-base font-bold text-foreground">{profile?.height || 170}</p>
              <p className="text-[10px] text-muted-foreground truncate">{labels.height}</p>
            </div>
            <div className="text-center p-2 rounded-xl bg-muted/50">
              <p className="text-base font-bold text-foreground">{profile?.weight || 70}</p>
              <p className="text-[10px] text-muted-foreground truncate">{labels.weight}</p>
            </div>
            <div className="text-center p-2 rounded-xl bg-muted/50">
              <p className="text-base font-bold text-foreground">{profile?.gender === 'male' ? '👨' : profile?.gender === 'female' ? '👩' : '👤'}</p>
              <p className="text-[10px] text-muted-foreground truncate">{labels.gender}</p>
            </div>
          </div>
        </motion.div>
