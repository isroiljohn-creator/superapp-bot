...
export const ReportsScreen: React.FC<ReportsScreenProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const data = dayKeys.map((dayKey, i) => {
      ...
      return {
        day: t(dayKey),
        water: isToday ? (todayLog?.water_ml || 0) / 100 : isPast ? Math.floor(Math.random() * 25) + 15 : 0,
        steps: isToday ? (todayLog?.steps || 0) / 1000 : isPast ? Math.floor(Math.random() * 10) + 5 : 0,
        sleep: isToday ? (todayLog?.sleep_hours || 0) : isPast ? Math.floor(Math.random() * 3) + 6 : 0,
      };
    });
