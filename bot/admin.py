...
"""
    ...
    @bot.message_handler(func=lambda message: message.text == "🎥 Mashqlar Bazasi")
    def handle_exercise_db(message):
        if message.from_user.id not in ADMIN_IDS: return
        
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("➕ Yangi Mashq Qo'shish", callback_data="ex_add"),
            types.InlineKeyboardButton("📋 Ro'yxatni Ko'rish", callback_data="ex_list")
        )
        bot.send_message(message.chat.id, "🎥 **Mashqlar Bazasi**\n\nTanlang:", reply_markup=markup, parse_mode="Markdown")

    @bot.callback_query_handler(func=lambda call: call.data == "ex_add")
    def ex_add_start(call):
        msg = bot.send_message(call.message.chat.id, "1️⃣ **Mashq nomini yozing:**\n(Masalan: Otjimaniya)", parse_mode="Markdown")
        bot.register_next_step_handler(msg, ex_save_name, bot)

    def ex_save_name(message, bot):
        name = message.text
        msg = bot.send_message(message.chat.id, "2️⃣ **Video havolasini yozing:**\n(YouTube yoki boshqa link)", parse_mode="Markdown")
        bot.register_next_step_handler(msg, ex_save_video, bot, name)

    def ex_save_video(message, bot, name):
        url = message.text
        # Ask for category
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        markup.add("Upper Body", "Lower Body", "Full Body", "Cardio")
        
        msg = bot.send_message(message.chat.id, "3️⃣ **Kategoriyani tanlang:**", reply_markup=markup, parse_mode="Markdown")
        bot.register_next_step_handler(msg, ex_save_category, bot, name, url)

    def ex_save_category(message, bot, name, url):
        category = message.text
    ...
    def ex_list_view(call):
        ...
        if not exercises:
             bot.send_message(call.message.chat.id, "Hozircha mashqlar yo'q.")
             return
             
        txt = "📋 **Mashqlar Ro'yxati:**\n\n"
        markup = types.InlineKeyboardMarkup()
        
        for ex in exercises:
            txt += f"• {ex['name']} ({ex['category']})\n"
            markup.add(types.InlineKeyboardButton(f"🗑 {ex['name']} ni o'chirish", callback_data=f"ex_del_{ex['id']}"))
            
        bot.send_message(call.message.chat.id, txt, reply_markup=markup, parse_mode="Markdown")

    @bot.callback_query_handler(func=lambda call: call.data.startswith("ex_del_"))
    def ex_delete_handler(call):
        try:
            ex_id = int(call.data.split("_")[2])
            if db.delete_exercise(ex_id):
                bot.answer_callback_query(call.id, "O'chirildi! ✅")
                # Refresh list (optional, simplistic here)
                bot.edit_message_text(f"🗑 Mashq (ID: {ex_id}) o'chirildi.", call.message.chat.id, call.message.message_id)
            else:
                bot.answer_callback_query(call.id, "Xatolik!")
        except:
            bot.answer_callback_query(call.id, "Xatolik!")
